...
const AMENITIES = [
  {
    ...
    description:
      "Sky-high relaxation with panoramic views of the city skyline.",
  },
  {
    id: 2,
    ...
    capacity: "12 Max",
    image:
      "https://images.unsplash.com/photo-1544161515-436cefb544de?auto=format&fit=crop&q=80&w=800",
    description:
      "Therapeutic treatments and hydrotherapy sessions in total privacy.",
  },
  {
    id: 3,
    ...
    description:
      "The ultimate social space for networking and executive leisure.",
  },
];
...
export default function ServicesPage() {
  ...
  const { searchQuery, setSearchQuery, filteredData } = useLocalFilter({
    data: AMENITIES,
    ...
    <Shell>
      ...
      >
        {[
          ...
          <AnimatedItem key={i}>
            <Surface
              level="container"
              className="p-6 transition-all hover:tier-2 group cursor-pointer border border-white/5"
            >
              <div className="flex justify-between items-start mb-6">
                ...
                <ChevronRight
                  size={14}
                  className="text-on-surface-variant/30 opacity-0 group-hover:opacity-100 transition-opacity"
                />
              </div>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-on-surface-variant mb-1">
                {service.label}
              </p>
              <h3 className="text-lg font-manrope font-extrabold uppercase tracking-widest">
                {service.count}
              </h3>
            </Surface>
          </AnimatedItem>
          ...
          {["ALL", "WELLNESS", "SOCIAL", "PRIVATE"].map((t) => (
            <button
              ...
              className={cn(
                ...
                activeTab === t
                  ? "bg-surface-container text-secondary-gold shadow-premium"
            ...
            >
              <div className="h-64 w-full relative overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  <span className="bg-surface-lowest/90 backdrop-blur-md text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-lg border border-white/10">
                    {amenity.category}
                  </span>
                  <div
                    className={cn(
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="font-manrope font-extrabold text-xl group-hover:text-secondary-gold transition-colors leading-tight mb-2 pr-4">
                      {amenity.name}
                    </h3>
                ...
                <div className="grid grid-cols-2 gap-4 pt-6 border-t border-white/5">
                  <div className="flex flex-col">
                    <span className="text-[9px] font-bold text-on-surface-variant uppercase tracking-widest mb-0.5">
                      Asset Load
                    </span>
                      ...
                      <div className="flex-1 h-1 bg-surface-lowest rounded-full overflow-hidden">
                        <div
                          className={cn(
                            ...
                            amenity.occupancy === "Low"
                              ? "bg-primary-emerald w-1/4"
                              : amenity.occupancy === "Normal"
                                ? "bg-secondary-gold w-1/2"
                                : "bg-red-500 w-full",
                          )}
                        />
                      </div>
      ...
      <section className="mt-16">
        <div className="flex items-center gap-4 mb-8">
          <h2 className="font-manrope font-bold text-2xl text-foreground uppercase tracking-[0.1em]">
            24h Operational <span className="text-primary-emerald">Flow</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <Surface
              key={i}
              level="low"
              className="p-6 group hover:tier-2 transition-all border border-white/5 border-b-2 border-b-transparent hover:border-b-primary-emerald"
            >
              <div className="flex justify-between items-center mb-6">
                <div className="p-2.5 rounded-xl bg-surface-lowest group-hover:scale-110 transition-transform">
                  <Clock
                    size={16}
                    className="text-on-surface-variant/50 group-hover:text-primary-emerald"
                  />
                </div>
              ...
              <div className="flex justify-between items-end">
                <div>
                  <h4 className="text-xs font-extrabold uppercase tracking-[0.15em] mb-1">
                    Peak Utilization
                  </h4>
                  <p className="text-2xl font-manrope font-black tracking-tighter">
                    1{i + 4}:00{" "}
                ...
                <ArrowUpRight
                  size={18}
                  className="text-primary-emerald/30 group-hover:text-primary-emerald transition-colors"
                />
              </div>
            </Surface>
          ))}
      ...
      <section className="mt-12 flex justify-center">
        <button className="flex items-center gap-4 bg-surface-lowest text-on-surface-variant hover:text-foreground px-8 py-4 rounded-full border border-white/5 transition-all hover:scale-105 active:scale-95 shadow-premium group">
          <ShieldCheck
            size={20}
            className="group-hover:text-secondary-gold transition-colors"
          />
          <span className="text-xs font-extrabold uppercase tracking-widest">
            Verify Safety Certification Nodes
          </span>
          <ArrowUpRight
            size={16}
