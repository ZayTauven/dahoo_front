  ...
  return (
    <Shell>
      ...
      <AnimatedSection stagger className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
        {[
          { label: "Visits Planned", value: "04", trend: "Today", icon: Users, color: "text-blue-400" },
          { label: "Deliveries Held", value: "12", trend: "Pending", icon: Package, color: "text-secondary-gold" },
          { label: "Alerts Level", value: "NORMAL", trend: "Optimal", icon: AlertTriangle, color: "text-primary-emerald" },
          { label: "Shift Progress", value: "6h", trend: "Remaining", icon: Clock, color: "text-on-surface-variant" },
        ].map((stat, i) => (
          <AnimatedItem key={i}>
            <Surface level="container" className="p-6 transition-all hover:tier-2 group">
              ...
              <p className="text-[9px] font-bold text-on-surface-variant/50 uppercase tracking-widest mt-1">{stat.trend}</p>
            </Surface>
          </AnimatedItem>
        ))}
      </AnimatedSection>
          ...
          <AnimatedSection stagger className="space-y-4">
            {filteredData.map((event) => (
              <AnimatedItem key={event.id}>
                <Surface level="container" className="p-4 group cursor-pointer hover:tier-2 transition-all no-line border-l-4 border-l-transparent hover:border-l-primary-emerald">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div className="flex items-center gap-4 min-w-80">
                      <div className="w-12 h-12 rounded-xl bg-surface-lowest flex items-center justify-center text-primary-emerald font-extrabold text-sm border border-white/5 group-hover:scale-105 transition-transform">
                        {event.type === "VISIT" ? <Users size={20} /> : event.type === "DELIVERY" ? <Package size={20} /> : <AlertTriangle size={20} />}
                      </div>
                      <div>
                        <h4 className="font-manrope font-extrabold text-lg group-hover:text-primary-emerald transition-colors">{event.subject}</h4>
                        <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant flex items-center gap-1.5">
                          <Home size={12} className="text-secondary-gold" /> {event.unit}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="w-32 text-right">
                      <p className="text-[10px] font-extrabold uppercase tracking-widest text-on-surface-variant mb-1">{event.time}</p>
                      <span className={cn(
                        "text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-widest",
                        event.status === "COMPLETED" || event.status === "CONFIRMED" ? "bg-primary-emerald/10 text-primary-emerald" : "bg-secondary-gold/10 text-secondary-gold"
                      )}>
                        {event.status}
                      </span>
                    </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-8">
          <Surface level="high" className="p-8 relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col gap-6">
              <h3 className="font-manrope font-extrabold text-xl tracking-tight">Rapid Reporting</h3>
              <p className="text-xs text-on-surface-variant font-medium leading-relaxed">Logged in as Guardian Vance. Ready to confirm field events for Hub Elite.</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <button className="p-5 rounded-2xl bg-surface-lowest border border-white/5 text-center flex flex-col items-center gap-3 active:scale-95 transition-all hover:bg-surface-container">
                  ...
                  <span className="text-[10px] font-extrabold uppercase tracking-widest text-foreground">Confirm Visit</span>
                </button>
                <button className="p-5 rounded-2xl bg-surface-lowest border border-white/5 text-center flex flex-col items-center gap-3 active:scale-95 transition-all hover:bg-surface-container">
                  <Zap size={28} className="text-secondary-gold" />
                  <span className="text-[10px] font-extrabold uppercase tracking-widest text-foreground">Active Task</span>
                </button>
              </div>

              <div className="flex flex-col gap-2">
                <Surface level="lowest" className="p-4 flex items-center justify-between group cursor-pointer hover:bg-surface-container transition-all">
                   <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-surface-high flex items-center justify-center text-primary-emerald">
                      <Calendar size={14} />
                    </div>
                    <span className="text-[10px] font-bold uppercase tracking-widest">Planned Agenda</span>
                   </div>
                ...
                <Surface level="lowest" className="p-4 flex items-center justify-between group cursor-pointer hover:bg-surface-container transition-all">
                   <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest">Urgent Alerts</span>
                   </div>
          ...
          <Surface level="low" className="p-8">
            <h3 className="font-manrope font-extrabold text-lg mb-6 tracking-tight">Operational Heartbeat</h3>
            <div className="space-y-6">
              {[
                { label: "Connectivity", status: "ESTABLISHED", icon: Zap, color: "text-primary-emerald" },
                { label: "Sync Latency", status: "14ms", icon: Clock, color: "text-blue-400" },
                { label: "RFID Nodes", status: "128 Active", icon: CheckCircle2, color: "text-secondary-gold" },
              ].map((node, i) => (
                <div key={i} className="flex justify-between items-center border-b border-white/5 pb-4 last:border-0 last:pb-0">
                  <div className="flex items-center gap-3">
                    <node.icon size={16} className={node.color} />
                    <span className="text-[10px] font-bold uppercase text-on-surface-variant tracking-wider">{node.label}</span>
                  </div>
                  <span className="text-[10px] font-extrabold text-foreground">{node.status}</span>
                </div>
              ))}
            </div>
          </Surface>
